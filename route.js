...
async function runAgent() {
  ...
  await log(`🔎 Mulai cari produk. Negara: ${countries.join(",")} · kata kunci: ${keywords.join(",") || "semua"}`);
  ...
  for (const p of found) {
    ...
    await log(`🧠 "${p.title}" → skor ${a.score} → ${a.platform} · $${a.daily_budget}/hari`);
...
// Dipanggil Vercel Cron tiap 6 jam
